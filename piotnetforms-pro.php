...
define( 'PIOTNETFORMS_PRO_VERSION', '2.1.34' );
...
class Piotnetforms_pro extends Piotnetforms_Variables_Pro {
	...
	public function init() {
        ...
        require_once( __DIR__ . '/inc/forms/ajax-razorpay-get-plan.php' );
        ...
        require_once( __DIR__ . '/inc/forms/ajax-twilio-sendgrid-get-field.php' );
	...
	public function enqueue_frontend_all() {
        ...
        if(empty(get_option( 'piotnetforms_disable_font_awesome' ))){
	...
	public function enqueue() {
        ...
        wp_localize_script($this->slug . '-advanced-script', 'piotnetforms_pro_advanced_nonce_obj', ['nonce' => wp_create_nonce('piotnetforms_pro_advanced_nonce')]);
        ...
        wp_localize_script($this->slug . '-advanced2-script', 'piotnetforms_pro_advanced2_nonce_obj', ['nonce' => wp_create_nonce('piotnetforms_pro_advanced2_nonce')]);
	...
	public function add_admin_scripts( $hook ) {
		...
		if ( isset( $_GET['page'] ) && isset( $_GET['post'] ) ) {
			if ( $_GET['page'] == 'piotnetforms' ) {
                ...
                wp_localize_script($this->slug . '-editor-script', 'piotnetforms_pro_editor_nonce_obj', ['nonce' => wp_create_nonce('piotnetforms_pro_editor_nonce')]);
				...
				wp_localize_script($this->slug . '-editor-forms-script', 'piotnetforms_pro_preview_nonce_obj', ['nonce' => wp_create_nonce('piotnetforms_pro_preview_nonce')]);
        ...
        wp_enqueue_style( $this->slug . '-admin-logo', plugin_dir_url( __FILE__ ) . 'assets/css/minify/logo.min.css', false, PIOTNETFORMS_PRO_VERSION );
        ...
        if($this->check_admin_enqueue()){
            ...
            wp_localize_script($this->slug . '-admin-script', 'piotnetforms_pro_admin_nonce_obj', ['nonce' => wp_create_nonce('piotnetforms_pro_admin_nonce')]);
        }
	}
					...
					$(document).on('click', '[data-edit-with-piotnetforms]', function(e){
						...
						if (status === 'auto-draft') {
							var data = {
                                ...
                                nonce: '<?php echo wp_create_nonce('piotnetforms_pro_auto_draft_nonce') ?>',
	...
	public function piotnet_base_settings() {
        ...
        register_setting( 'piotnetforms-razorpay-group', 'piotnetforms-razorpay-key-id' );
        register_setting( 'piotnetforms-razorpay-group', 'piotnetforms-razorpay-key-secret' );
        ...
        register_setting( 'piotnetforms-general-group', 'piotnetforms_disable_font_awesome' );
	}
	...
	public function piotnetforms_pdf_metabox_output( $post ) {
		...
		$html = '<div class="piotnetforms-custom-font">
			...
			<button type="button" id="piotnetforms-pdf-upload-font" class="button">Upload/Add font</button>
			<button type="button" id="piotnetforms-pdf-remove-font" class="button">Remove font</button>
		</div>';
		echo $html;
	}
