	...
	if ($uni) {
		...
		if (!isset($type) ||  !isset($name) || $originalsize != $ttfstat['size']) {
			...
			if (is_writable(dirname($this->fontpath.'unifont/'.'x'))) {
				$fh = fopen(wp_upload_dir()['basedir']. '/' . $unifilename.'.mtx.php',"w");
				fwrite($fh,$s,strlen($s));
				fclose($fh);
				$fh = fopen(wp_upload_dir()['basedir']. '/' . $unifilename.'.cw.dat',"wb");
				fwrite($fh,$cw,strlen($cw));
				fclose($fh);
				@unlink(wp_upload_dir()['basedir']. '/' . $unifilename.'.cw127.php');
			}
			unset($ttf);
		}
		else {
