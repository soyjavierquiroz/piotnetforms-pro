<?php
	...
	function replace_email_piotnetforms( $content, $fields, $payment_status = 'succeeded', $payment_id = '', $succeeded = 'succeeded', $pending = 'pending', $failed = 'failed', $submit_id = 0 ) {
		...
		if ( !empty( $fields ) ) {
			...
			foreach ( $fields as $field ) {
				...
				if ( !empty( $repeater_id ) && !empty( $repeater_label ) ) {
					...
					if ( strpos( $field['name'], 'piotnetforms-end-repeater' ) === false ) {
						if ( $field['image_upload'] == true || ( !empty($field['type']) && $field['type'] == 'file' )) {
			...
			foreach ( $fields as $field ) {
                ...
                if(isset( $_POST['mollie_payment']) && !empty($field['calculation_results'])){
                    $message = $field['calculation_results'];
                }
			}
		}
        ...
        $piotnetforms_submit_id = 0;
		$piotnetforms_submit_id = !empty($GLOBALS['piotnetforms_submit_id']) ? $GLOBALS['piotnetforms_submit_id'] : 0;
		...
		if ( !empty( $_POST['post_id'] ) && !empty( $_POST['form_id'] ) && !empty( $_POST['fields'] ) ) {
			...
			foreach ( $fields as $key_field=>$field ) {
				...
				if ( isset( $fields[$key_field]['attach-files'] ) ) {
					if ( $fields[$key_field]['type'] == 'text' || $fields[$key_field]['type'] == 'signature') {
					...
					if ( $fields[$key_field]['attach-files'] == 1 ) {
						if ( isset( $fields[$key_field] ) && $fields[$key_field]['type'] !== 'signature') {
							unset( $fields[$key_field] );
						}else{
                            $signature_delete[] = $key_field;
                        }
					}
				}
                //Check email exists
                if(!empty($form['settings']['check_email_exists']) && isset($field['type']) && $field['type'] == 'email'){
                    $check_all = !empty($form['settings']['check_email_exists_for']) && $form['settings']['check_email_exists_for'] == 'all-form' ? true : false;
                    $email_exists = $helper->piotnetforms_check_email_exists($form_id, $field['name'], $field['value'], $check_all);
                    $email_exists_msg = !empty($form['settings']['email_exists_notification']) ? $form['settings']['email_exists_notification'] : 'Email [email] already exists.';
                    $email_exists_msg = str_replace('[email]', $field['value'], $email_exists_msg);
                    if($email_exists){
                        echo json_encode([
                            'status' => 'email_exists',
                            'email_name' => $field['name'],
                            'email' => $field['value'],
                            'msg_email_exists' => $email_exists_msg
                        ]);
                        wp_die();
                    }
                }
			...
			if ( !empty( $_POST['mollie_payment'] ) ) {
				$mollie_amount_value = replace_email_piotnetforms( $form['settings']['mollie_amount'], $fields);
				$mollie_amount_value = preg_replace( '/[^0-9.,]+/', '', $mollie_amount_value );
				$mollie_amount_value = number_format( $mollie_amount_value, 2, '.', '');
                $mollie_description = !empty($form['settings']['mollie_description']) ? $form['settings']['mollie_description'] : '';
                $mollie_payment_desc = piotnetforms_get_field_value( $form['settings']['mollie_description'], $fields);
                $mollie_payment_desc = !empty($mollie_payment_desc) ? $mollie_payment_desc : $mollie_description;
				$mollie_payment_data = [
						'amount' => [
							'currency' => $form['settings']['mollie_currency'] ? $form['settings']['mollie_currency'] : 'USD',
							'value' => $mollie_amount_value
						],
						'description' => $mollie_payment_desc,
						'redirectUrl' => $_POST['mollie_redirect_url'],
						'locale' => !empty($form['settings']['mollie_locale']) ? $form['settings']['mollie_locale'] : 'en_US',
					];
				foreach ( $fields as $key => $value ) {
					$mollie_payment_data['metadata'][$value['name']] = $value['value'];
				}
                $mollie_payment_metadata = http_build_query( $mollie_payment_data['metadata']);
                $mollie_metadata_size = strlen($mollie_payment_metadata);
                if($mollie_metadata_size > 1024){
                    unset($mollie_payment_data['metadata']);
                }
            ...
            if(!empty( $form['settings']['razorpay_enable'] ) && $_POST['razo_payment'] == 'yes'){
                $razor_id = get_option( 'piotnetforms-razorpay-key-id' );
                $razor_secret = get_option( 'piotnetforms-razorpay-key-secret' );
                $razor_notes = [];
                if(!empty($form['settings']['razorpay_notes']) && !empty($form['settings']['razorpay_note_list'])){
                    foreach($form['settings']['razorpay_note_list'] as $k => $metadata){
                        ...
                        $razor_notes[$razor_note_key] = replace_email_piotnetforms($metadata['razorpay_metadata_value'],$fields);
                    }
                }
                $razor_amount = replace_email_piotnetforms($form['settings']['razorpay_amount'],$fields);
                $razor_amount = floatval($razor_amount) * 100;
                $razor_prefill = [];
                if(!empty($form['settings']['razorpay_prefill'])){
                    $razor_prefill = [
                        'name' => replace_email_piotnetforms($form['settings']['razorpay_customer_name'],$fields),
                        'email' => replace_email_piotnetforms($form['settings']['razorpay_customer_email'],$fields),
                        'contact' => replace_email_piotnetforms($form['settings']['razorpay_customer_contact'],$fields),
                    ];
                }
                $razor_payment_data = [
                    'key' => $razor_id,
                    'amount' => $razor_amount,
                    'currency' => !empty($form['settings']['razorpay_currency']) ? $form['settings']['razorpay_currency'] : 'INR',
                    'name' => replace_email_piotnetforms($form['settings']['razorpay_name'],$fields),
                    'description' => replace_email_piotnetforms($form['settings']['razorpay_description'],$fields),
                    'image' => replace_email_piotnetforms($form['settings']['razorpay_image'],$fields),
            ...
            if(! empty( $form['settings']['razorpay_sub_enable'] ) && $_POST['razo_sub'] == 'yes'){
                $razorpay_id = get_option( 'piotnetforms-razorpay-key-id' );
                $razorpay_secret = get_option( 'piotnetforms-razorpay-key-secret' );
                $razor_quantity = replace_email_piotnetforms($form['settings']['razor_subcription_quantity'],$fields);
                $razor_total_count = replace_email_piotnetforms($form['settings']['razor_subcription_total_count'],$fields);
                $razor_sub_prefill = [];
                if(!empty($form['settings']['razor_sub_prefill'])){
                    $razor_sub_prefill = [
                        'name' => replace_email_piotnetforms($form['settings']['razorpay_sub_customer_name'],$fields),
                        'email' => replace_email_piotnetforms($form['settings']['razorpay_sub_customer_email'],$fields),
                        'contact' => replace_email_piotnetforms($form['settings']['razorpay_sub_customer_contact'],$fields),
                    ];
                }
                $razor_sub_notes = [];
                if(!empty($form['settings']['razorpay_sub_notes']) && !empty($form['settings']['razorpay_sub_note_list'])){
                    foreach($form['settings']['razorpay_sub_note_list'] as $k => $metadata){
                        ...
                        $razor_sub_notes[$razor_note_key] = replace_email_piotnetforms($metadata['razorpay_sub_metadata_value'],$fields);
                    }
                }
                $razorpay_sub_url = 'https://api.razorpay.com/v1/subscriptions';
                $razor_sub = [
                    'plan_id' => replace_email_piotnetforms($form['settings']['razor_subcription_plan_id'],$fields),
                ...
                if ( !is_wp_error( $razor_sub_response ) && !is_wp_error(wp_remote_retrieve_body($razor_sub_response)) ) {
                    ...
                    $razor_sub_data = [
                        ...
                        'name' => replace_email_piotnetforms($form['settings']['razor_subcription_name'],$fields),
                        'description' => replace_email_piotnetforms($form['settings']['razor_subcription_desc'],$fields),
                        'image' => replace_email_piotnetforms($form['settings']['razor_subcription_image'],$fields),
...
', $field['value'] ) ),
				...
				if ( in_array( 'pdfgenerator', $form['settings']['submit_actions'] ) ) {
						...
						foreach ( $pdf_generator_list as $item ) {
							...
							if ( $type == 'image' ) {
                                ...
                                if(wp_http_validate_url($pdf_image_url)){
                                    ...
                                    if(!empty($item['pdf_remove_signature'])){
                                        $signature_name = basename( $pdf_image_url );
							            $signature_url = wp_get_upload_dir()['path'] . '/'. $signature_name;
                                        piotnetforms_delete_acttachment($signature_url);
                                        $signature_field_name = str_replace(['[field id="', '"]'], ['', ''], $item['pdfgenerator_field_shortcode']);
                                        foreach($fields as $key => $value){
                                            if($value['name'] == $signature_field_name){
                                                unset($fields[$key]);
                                            }
                                        }
                                    }
                    ...
                    //Delete signature
                    if(!empty($signature_delete)){
                        foreach($signature_delete as $value){
                            unset($fields[$value]);
                        }
                    }
				}
				...
				if ( in_array( 'twilio_whatsapp', $form['settings']['submit_actions'] ) ) {
					if ( !empty( $form['settings']['whatsapp_form'] ) && !empty( $form['settings']['whatsapp_to'] ) && !empty( $form['settings']['whatsapp_message'] ) ) {
                        ...
                        foreach($whatsapp_to as $number){
                            ...
                            $helper = new piotnetforms_Helper();
                            $whatapps_response = $helper->piotnetforms_twilio_whatsapp_send_msg($whatsapp_data, $whatsapp_url, $whatsapp_header);
                        }
					}
				}
				...
				if ( in_array( 'booking', $form['settings']['submit_actions'] ) ) {
						...
						if ( !empty( $field['booking'] ) ) {
							...
							foreach ( $booking as $booking_key => $booking_item ) {
								...
								if ( !empty( $booking_item['piotnetforms_booking_date_field'] ) ) {
                                    $date = date_create_from_format($booking_item['date_format'], replace_email_piotnetforms( $booking_item['piotnetforms_booking_date_field'], $fields ));
					...
					foreach ( $piotnetforms_booking as $booking ) {
                        if ( $booking['piotnetforms_booking_date_type'] === 'date_picker') {
                            $date = date_create_from_format($booking_item['date_format'], replace_email_piotnetforms( $booking_item['piotnetforms_booking_date_field'], $fields ));
                            $date = date_format($date, 'Y-m-d');
                            if(!$date){
                ...
                //Webhook
                if ( in_array( 'webhook', $form['settings']['submit_actions'] ) && !empty( $form['settings']['webhooks'] ) && $webhook) {
				...
				if ( in_array( 'email', $form['settings']['submit_actions'] ) && $failed == false ) {
					...
					if ( empty( $reply_to ) ) {
						$reply_to = ( ! empty( $form['settings']['email_from'] ) ) ?  replace_email_piotnetforms( $form['settings']['email_from'], $fields, '', '', '', '', '', $form_database_post_id ) : '';
					}
                    ...
                    // Attach default file
                    if(!empty($form['settings']['enable_attachment_default_email']) && !empty($form['settings']['attachment_default_url'])){
                        $default_files = explode(',', $form['settings']['attachment_default_url']);
                        foreach($default_files as $file_url){
                            $default_file_dir = str_replace(WP_CONTENT_URL, WP_CONTENT_DIR, $file_url);
                            array_push($attachment, $default_file_dir);
                        }
                    }
				...
				if ( in_array( 'email2', $form['settings']['submit_actions'] ) && $failed == false ) {
					...
					if ( empty( $reply_to ) ) {
						$reply_to = ( ! empty( $form['settings']['email_from_2'] ) ) ? replace_email_piotnetforms( $form['settings']['email_from_2'], $fields, '', '', '', '', '', $form_database_post_id ) : '';
					}
					$reply_to = replace_email_piotnetforms( $reply_to, $fields, '', '', '', '', '', $form_database_post_id );
                    ...
                    // Attach default file
                    if(!empty($form['settings']['enable_attachment_default_email2']) && !empty($form['settings']['attachment_email2_default_url'])){
                        $default_files = explode(',', $form['settings']['attachment_email2_default_url']);
                        foreach($default_files as $file_url){
                            $default_file_dir = str_replace(WP_CONTENT_URL, WP_CONTENT_DIR, $file_url);
                            array_push($attachment, $default_file_dir);
                        }
