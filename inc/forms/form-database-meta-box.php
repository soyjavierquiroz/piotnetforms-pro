...
function form_database_meta_box_output_piotnetforms() {
	...
	if ( $fields_database ) {
		$fields_database = strip_tags( $fields_database, ['<br>', '<strong>']);
	...
	if ( $fields_database ) {
		foreach ( $fields_database as $key => $field ) {
			if ( $key != 'form_id_piotnetforms' && $key != 'post_id' && $key != '_edit_lock' && $key != '_piotnetforms_fields_database' ) {
				...
				echo '<tr>';
				echo '<td style="padding: 15px;border: 1px solid #ccc;font-weight: 700">';
				echo $label;
				echo '</td>';
				echo '<td style="padding: 15px;border: 1px solid #ccc;">';
				if ( $key == 'payment_amount' ) {
					echo $field['value'] / 100;
				} else {
					echo nl2br( str_replace(['<br />n', '<br/>n', '<br>n'], ['<br>', '<br>', '<br>'], $field['value']) );
				}
				echo '</td>';
				echo '</tr>';
			}
