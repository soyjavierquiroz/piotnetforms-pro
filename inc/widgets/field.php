...
class Piotnetforms_Field extends Base_Widget_Piotnetforms {
	...
	private function add_settings_controls() {
        ...
        $this->add_control(
			'max_files_notification',
			[
				'label'       => __( 'Max Files Message', 'piotnetforms' ),
				'type'        => 'text',
				'value'       => __( 'Please do not exceed [max_files] files.', 'piotnetforms' ),
				'render_type' => 'none',
                'label_block' => true,
				'condition' => [
					'field_type'     => 'upload',
        ...
        $this->add_control(
			'minimum_files',
			[
				'label'     => __( 'Minimum Files', 'piotnetforms' ),
				'type'      => 'number',
				'condition' => [
					'field_type'            => 'upload',
					'allow_multiple_upload' => 'true',
        ...
        $this->add_control(
			'minimum_files_notification',
			[
				'label'       => __( 'Minimum Message', 'piotnetforms' ),
				'type'        => 'text',
				'value'       => __( 'Please upload at least [minimum_files] files.', 'piotnetforms' ),
				'render_type' => 'none',
                'label_block' => true,
				'condition' => [
					'field_type'     => 'upload',
		...
		$this->add_control(
			...
			[
				...
				'type'      => 'switch',
				'condition' => [
					'field_type' => ['upload', 'image_upload', 'signature']
				],
			]
		);
	...
	private function add_image_select_style_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'   => [
                    ...
                    '.piotnetforms-image-select-preview .thumbnail' => 'text-align: {{VALUE}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'   => [
                    ...
                    '.piotnetforms-image-select-preview .thumbnail p' => 'text-align: {{VALUE}};'
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
                    ...
                    '.piotnetforms-image-select-preview ul.thumbnails.image_picker_selector li' => 'width: {{VALUE}}% !important;',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'   => [
                    ...
                    '.piotnetforms-image-select-preview .piotnetforms-image-select-field .image_picker_selector' => 'justify-content: {{VALUE}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'  => [
                    ...
                    '.piotnetforms-image-select-preview ul.thumbnails.image_picker_selector' => 'margin: ' . $columns_margin,
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'  => [
                    ...
                    '.piotnetforms-image-select-preview ul.thumbnails.image_picker_selector .thumbnail' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'  => [
                    ...
                    '.piotnetforms-image-select-preview ul.thumbnails.image_picker_selector .image_picker_image' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'  => [
                    ...
                    '.piotnetforms-image-select-preview .image_picker_image' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'  => [
                    ...
                    '.piotnetforms-image-select-preview ul.thumbnails p' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$normal_controls = $this->add_image_select_thumbnail_style(
			'normal',
			[
				'selectors' => '{{WRAPPER}} ul.thumbnails.image_picker_selector .thumbnail, .piotnetforms-image-select-preview ul.thumbnails.image_picker_selector .thumbnail',
			]
		);
		$this->add_control(
	...
	protected function make_select_field( $item, $i, $form_id, $image_select = false, $terms_select = false, $select_autocomplete = false, $select2 = false ) {
		...
		$options = preg_split( '/\\r\\n|\\r|\\n/', !empty($item['field_options']) ? $item['field_options'] : '' );
		...
		if ( empty( $value ) ) {
			$this->add_render_attribute( 'select' . $i, 'data-piotnetforms-default-value', !empty($item['field_value']) ? $item['field_value'] : '' );
		} ?>
		...
		if ( $options ) {
			...
			foreach ( $options as $key => $option ) {
				...
				if ( $item['piotnetforms_style_checkbox_replacement'] && $item['piotnetforms_style_checkbox_type'] == 'native' ) {
					$icon_html = $this->piotnetforms_checkbox_radio_replacement($item);
				}
			...
			$html .= '</div>';
            echo '<script type="text/javascript">if(void 0===clearValidity)function clearValidity(i){jQuery(i).closest(".piotnetforms-field-subgroup").find(".piotnetforms-field-option input").each((function(){jQuery(this)[0].setCustomValidity("")}))}</script>';
			// $html .= '</form>';
		}
    ...
    private function piotnetforms_checkbox_radio_replacement($item){
        $icon_html = '';
        if ( $item['piotnetforms_style_checkbox_replacement_icon_type'] == 'font_awesome' ) {
            if ( !empty($item['piotnetforms_style_checkbox_replacement_icon_font_awesome']) || !empty($item['piotnetforms_style_checkbox_replacement_icon_font_awesome_checked']) ) {
                wp_enqueue_style( $this->slug . '-fontawesome-style' );
	...
	protected function form_fields_render_attributes( $i, $instance, $item ) {
		...
		if ( $item['field_type'] == 'image_upload' || $item['field_type'] == 'signature') {
	...
	public function render() {
		...
		if ( !empty( $settings['piotnetforms_conditional_logic_form_list'] ) ) {
			...
			if ( !empty( $settings['piotnetforms_conditional_logic_form_enable'] ) && !empty( $list_conditional[0]['piotnetforms_conditional_logic_form_if'] ) && !empty( $list_conditional[0]['piotnetforms_conditional_logic_form_comparison_operators'] ) ) {
                ...
                if(!empty($settings['conditional_visibility_action']) && $settings['conditional_visibility_action'] == 'show'){
                    $this->add_render_attribute( 'wrapper', 'style', 'display:none;' );
                }
		...
		switch ( $item['field_type'] ) :
			...
			case 'image_upload':
				$name = $this->get_field_name_shortcode( $this->get_attribute_name( $item ) );
				$value = $this->get_value_edit_post( $name );
                $max_image_size = !empty($item['file_sizes']) ? $item['file_sizes'] : 'false';
                $max_image_size_msg = !empty($item['file_sizes_message']) ? $item['file_sizes_message'] : '';

                if(!wp_script_is($this->slug . '-fontawesome-style')){
                    wp_enqueue_style( $this->slug . '-fontawesome-style' );
                }
                ...
                if ( ! empty( $item['minimum_files'] ) && ! empty( $item['allow_multiple_upload'] ) ) {
                    $upload_minimum_msg = !empty( $item['upload_minimum_msg'] ) ? $item['upload_minimum_msg'] : __( 'Please upload at least [minimum_files] files', 'piotnetforms');
                    $upload_minimum_msg = str_replace( '[minimum_files]', $item['minimum_files'], $upload_minimum_msg );
                    $this->add_render_attribute( 'input' . $item_index, 'data-piotnetforms-image-upload-minimum-files-msg',  $upload_minimum_msg );
                    $this->add_render_attribute( 'input' . $item_index, 'data-piotnetforms-image-upload-minimum-files',  $item['minimum_files'] );
				}

				echo '>';
				echo '<input type="file" accept="image/*" name="upload" style="display:none;" data-piotnetforms-max-size="'.$max_image_size.'" data-piotnetforms-max-size-msg="'.$max_image_size_msg.'"';
				if ( ! empty( $item['allow_multiple_upload'] ) ) {
					echo 'multiple="multiple"';
				}
			...
			case 'upload':
                ...
                if ( ! empty( $item['max_files'] ) ) {
                    ...
                    $this->add_render_attribute( 'input' . $item_index, 'data-piotnetforms-upload-max-files-message',  $notification_max_file);
				}

                

                if(! empty( $item['minimum_files'])){
                    $this->add_render_attribute( 'input' . $item_index, 'data-piotnetforms-upload-min-files',  $item['minimum_files']);
                    $notification_minimum_file = !empty($item['minimum_files_notification']) ? $item['minimum_files_notification'] : 'Please upload a minimum of [minimum_files] files.';
                    $this->add_render_attribute( 'input' . $item_index, 'data-piotnetforms-upload-minimum-files-message',  $notification_minimum_file);
                }
			...
			case 'number':
                ...
                if(!empty($item['max_length'])){
                    $this->add_render_attribute( 'input' . $item_index, 'onKeyPress', 'if(this.value.length=='.$item['max_length'].') return false;' );
                }
			...
			case 'acceptance':
				$label = '';
                $acceptance_value = !empty($item['field_value']) ? $item['field_value'] : 'yes';
				...
				$this->add_render_attribute( 'input' . $item_index, 'data-piotnetforms-id', $form_id );
				$this->add_render_attribute( 'input' . $item_index, 'value', $acceptance_value );

				if ( ! empty( $item['acceptance_text'] ) ) {
                    $icon_acceptance = $this->piotnetforms_checkbox_radio_replacement($item);
					$label = '<label for="' . $this->get_attribute_id( $item ) . '">'. $icon_acceptance . $item['acceptance_text'] . '</label>';
				}
                ...
                $acceptance_replacement_class = !empty($item['piotnetforms_style_checkbox_replacement']) && $item['piotnetforms_style_checkbox_type'] == 'native' ? ' piotnetforms-field-subgroup--checkbox-replacement' : '';
				echo '<div class="piotnetforms-field-subgroup piotnetforms-field-subgroup--' . $item['piotnetforms_style_checkbox_type'] . ' '. $acceptance_replacement_class .'"><span class="piotnetforms-field-option"><input ' . $this->get_render_attribute_string( 'input' . $item_index ) . '> ' . $label . '</span></div>';
				break;
			case 'date':
		...
		?>
			...
			<div <%= view.render_attributes('wrapper') %>>
				<div <%= view.render_attributes('field-group') %>>
					...
					<div class="piotnetforms-field-container<%= field_inline %>">
									...
									<form>
										...
										for ( i = 0; i < options.length; i++ ) {
											...
											if ( s['piotnetforms_style_checkbox_replacement'] && s['piotnetforms_style_checkbox_type'] == 'native' ) {
                                                icon_html = piotnetforms_checkbox_radio_replacement(s);
											}
						...
						<%
								...
								case 'acceptance':
                                    ...
                                    let acceptanceIcon = s['piotnetforms_style_checkbox_replacement'] == 'yes' ? piotnetforms_checkbox_radio_replacement(s) : '';
                                    let acceptance_replacement_class = s['piotnetforms_style_checkbox_replacement'] && s['piotnetforms_style_checkbox_type'] == 'native' ? ' piotnetforms-field-subgroup--checkbox-replacement' : '';
								%>
									<div class="piotnetforms-field-subgroup piotnetforms-field-subgroup--<%= s['piotnetforms_style_checkbox_type'] %><%= acceptance_replacement_class %> <%= s['inline_list'] %>"><span class="piotnetforms-field-option"><input <%= view.render_attributes('input') %>><% if ( s['acceptance_text'] ) { %><label for="<%= get_attribute_id( s ) %>"><%= acceptanceIcon + s['acceptance_text'] %></label><% } %></span></div>
								<%
									break;
								case 'date':
									view.add_attribute( 'input', 'data-piotnetforms-id', s.form_id );
                    ...
                    <%
                        function piotnetforms_checkbox_radio_replacement(s){
