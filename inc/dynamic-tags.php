<?php
	...
	function piotnetforms_dynamic_tags( $output, $post_id = '' ) {
		if ( isset($output) && stripos( $output, '{{' ) !== false && stripos( $output, '}}' ) !== false ) {
			$pattern = '~\{\{\s*(.*?)\s*\}\}~';
			...
			if ( ! empty( $dynamic_tags ) ) {
				foreach ( $dynamic_tags as $tag ) {
                    ...
                    if ( $tag['name'] == 'local_date_time' ) {
                        $local_time = current_time('timestamp');
						if ( empty( $tag['attr']['date_format'] ) ) {
							$tag_value = date( 'Y-m-d H:i:s', $local_time );
						} else {
							$tag_value = date( $tag['attr']['date_format'], $local_time );
						}
					}
